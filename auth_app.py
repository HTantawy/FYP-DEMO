...
def create_admin_user():
    ...
    try:
        ...
        cur.execute("SELECT id FROM users WHERE email = %s", ('htantawy@bham.ac.uk',))
        if not cur.fetchone():
            password_hash = hash_password('hussein1234')
            cur.execute("""
                INSERT INTO users (email, password_hash, full_name, user_type)
                VALUES (%s, %s::text, %s, %s)
            """, ('htantawy@bham.ac.uk', password_hash, 'Module Lead', 'admin'))
            conn.commit()
...
""")
    ...
    # Add after your existing table creations but before conn.commit()
        cur.execute("""
    CREATE TABLE IF NOT EXISTS supervisor_publications (
        id SERIAL PRIMARY KEY,
        supervisor_id INTEGER REFERENCES users(id),
        title TEXT NOT NULL,
        authors TEXT[],
...
""")
        ...
        cur.execute("""
    CREATE TABLE IF NOT EXISTS supervised_projects (
        id SERIAL PRIMARY KEY,
        supervisor_id INTEGER REFERENCES users(id),
        title TEXT NOT NULL,
        student_name TEXT,
...
# Add new columns to existing supervisor_profiles table
        cur.execute("""
...
def is_valid_email(email):
    ...
    pattern = r'^[a-zA-Z0-9._%+-]+@bham\.ac\.uk$'
    ...
    try:
        ...
        if user_type == 'supervisor' and additional_data:
            ...
            """, (
                ...
                additional_data.get('preferred_projects', []),
                additional_data.get('max_capacity', 5)
            ))
...
def login_page():
    ...
    st.markdown("""
        ...
        .stButton>button:last-child:active {
    ...
    with col2:
        st.markdown("""<div style="text-align: center; margin-bottom: 2px;">
                    <img src="https://i.ibb.co/khnRGSD/1.png" style="width: 200px; height: 180px;"/>
                </div>
            """, unsafe_allow_html=True)
        # Logo and title
        if svg_content:
    ...
    with col2:
        ...
        with st.form("supervisor_signup_form", clear_on_submit=True):
            ...
            with col2:
                ...
                max_value=20,
                value=5,
                help="Maximum number of students you can supervise simultaneously"
            )    
